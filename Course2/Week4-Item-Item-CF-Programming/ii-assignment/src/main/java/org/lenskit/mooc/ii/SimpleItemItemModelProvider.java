...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        for (long itemA: itemVectors.keySet()) {
            ...
            Long2DoubleMap vecA = itemVectors.get(itemA);

            for (long itemB: itemVectors.keySet()) {
                if (itemA != itemB) {
                    Long2DoubleMap vecB = itemVectors.get(itemB);

                    double cosine = Vectors.dotProduct(vecA, vecB)/ (Vectors.euclideanNorm(vecA)*Vectors.euclideanNorm(vecB));
