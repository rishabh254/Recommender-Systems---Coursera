    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                double termFreq = work.containsKey(tag) ? work.get(tag) + 1.0 : 1.0;
                work.put(tag, termFreq);

                if (!seenTags.contains(tag)) {
                    seenTags.add(tag);
                    double newFreq = docFreq.containsKey(tag) ? docFreq.get(tag) + 1.0 : 1.0;
                    docFreq.put(tag, newFreq);
                }
            }
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            for (String term : tv.keySet()) {
                ...
                double tfidf = tv.get(term)*docFreq.get(term);
                tv.put(term, tfidf);
            ...
            for (String term : tv.keySet()) {
                double normalizedTfidfValue = tv.get(term) / sumOfSquares;
                tv.put(term, normalizedTfidfValue);
            }
