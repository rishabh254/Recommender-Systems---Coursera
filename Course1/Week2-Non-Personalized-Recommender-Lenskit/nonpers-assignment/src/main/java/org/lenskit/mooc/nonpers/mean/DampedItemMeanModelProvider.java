...
public class DampedItemMeanModelProvider implements Provider<ItemMeanModel> {
    ...
    public ItemMeanModel get() {
        ...
        TreeMap<Long, List<Double>> globalItemRatings = new TreeMap<Long, List<Double>>();
        ...
        try (ObjectStream<Rating> ratings = this.dao.query(Rating.class).stream()) {
            ...
            for (Rating r : ratings) {
                if (!globalItemRatings.containsKey(r.getItemId()))
                    globalItemRatings.put(r.getItemId(), new LinkedList<Double>());

                List<Double> itemRatings = globalItemRatings.get(r.getItemId());
                itemRatings.add(r.getValue());
        ...
        for (long itemId : globalItemRatings.keySet()) {
            List<Double> itemRatings = globalItemRatings.get(itemId);

            double sum = 0;
            for(int i=0;i<itemRatings.size();i++)
                sum+=itemRatings.get(i);

            double dampedMean = (sum + this.damping * globalMeanRating) / (itemRatings.size()+ this.damping);
